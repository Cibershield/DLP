...
try:
    ...
    print("⚠️  inotify no disponible. Instalar con: pip install inotify")
        ...
        for proc in psutil.process_iter(['pid', 'name', 'cmdline', 'username', 'cwd', 'ppid']):
            try:
                ...
                if self._is_github_git_command(cmd_str, name):
                    ...
                    # Verificar si es un repo de Delfix-CR
                    is_delfix_repo = False
                    if target_url and ("delfix-cr" in target_url.lower() or "delfix_cr" in target_url.lower()):
                        is_delfix_repo = True
                    ...
                    if is_delfix_repo:
                        self.logger.warning(
                            f"👀 ACCESO DELFIX-CR DETECTADO: {event.username} -> {target_url}"
                        )
...
class FileSystemMonitor:
        ...
        try:
            ...
            for event in self.notifier.event_gen(yield_nones=False):
                ...
                if 'IN_ISDIR' in type_names and filename == '.git':
                    ...
                    # Intentar leer config para ver si es de Delfix
                    target_url = self._get_repo_origin(full_path)
                    is_delfix_repo = False
                    if target_url and ("delfix-cr" in target_url.lower() or "delfix_cr" in target_url.lower()):
                        is_delfix_repo = True
    ...
    def _get_repo_origin(self, git_dir: str) -> Optional[str]:
        """Intenta leer la URL del origin del config de git"""
        try:
            ...
            with open(config_path, "r", errors='ignore') as f:
                content = f.read()
                # Búsqueda simple de url = ...
                # [remote "origin"]
                # 	url = https://github.com/StartDD/antigravity.git
...
class DLPAgent:
    """Agente principal que coordina todos los monitores - Versión Optimizada"""
    ...
    def start(self):
        ...
        self.logger.info("=" * 60)
        self.logger.info("GitHub DLP Agent v2.0 (Optimizado) iniciando...")
        ...
        self.logger.info("-" * 60)
        self.logger.info("Límites de recursos:")
        ...
        self.logger.info("✓ Todos los monitores activos:")
        self.logger.info("  - ProcessMonitor (comandos git)")
        self.logger.info("  - FileSystemMonitor (carpetas .git)")
        self.logger.info("  - NetworkMonitor (conexiones a GitHub)")
        self.logger.info("  - MetricsReporter (rendimiento)")
        self.logger.info("")
